        ...
        for (EntityType t : cropTypes) {
            FXGL.getGameWorld().getEntitiesByType(t).stream()
                    ...
                    .findFirst().ifPresent(c -> {
                        CropComponent crop = c.getComponent(CropComponent.class);
                        CropData cropData = crop.getData();

                        // Cây chuyển sang trạng thái "đã hái" - hiện harvest.png (cây trống quả)
                        crop.harvest();

                        // Đất vẫn giữ nguyên, không reset hasPlant
                        ItemType res = ItemType.valueOf(t.name());
                        inventory.addItem(res, cropData.yield);
                        System.out.println("Thu hoạch " + res.getDisplayName() + " x" + cropData.yield);
